...
// constructors
////////////////////////////////////////////////////////////////////////////////
scmval s_fix(fixnum i) {
    scmval v = make_val(SCM_TYPE_FIXNUM);
    v.i = i;
    return v;
}

scmval s_flo(flonum d) {
...
static scmval scm_finite_p(scmval x) {
    return s_bool(!(is_nan(x) || is_pos_inf(x) || is_neg_inf(x)));
}

static scmval scm_infinite_p(scmval x) {
    return s_bool(is_pos_inf(x) || is_neg_inf(x));
}

static scmval scm_nan_p(scmval x) {
    return s_bool(is_nan(x));
}
...
static scmval scm_zero_p(scmval x) {
    ...
    return s_bool((is_fixnum(x) && c_fix(x) == 0) || (is_flonum(x) && c_flo(x) == 0.0));
}
...
static scmval scm_div(int argc, scmval* argv) {
    check_args("/", number_c, argc, argv);
    if(argc == 1) return ndiv(s_fix(1), argv[0]);
    scmval x = argv[0];
    for(int i = 1; i < argc; i++) {
...
static scmval scm_inexact(scmval x) {
    ...
    scmval ret = x;
    if(is_fixnum(x)) {
        ret = s_flo((flonum)c_fix(x));
    } else if(is_bignum(x)) {
        ret = s_flo(mpz_get_d(c_big(x)));
    }
    return ret;
}
...
void init_number(scmval env) {
    ...
    scm_0       = s_fix(0);
    scm_pos_inf = s_flo(HUGE_VAL);
    scm_neg_inf = s_flo(-HUGE_VAL);
    scm_nan     = s_flo(NAN);
}
    ...
    switch(tx) {
        case SCM_TYPE_FLONUM:
            switch(ty) {
                ...
                case SCM_TYPE_BIGNUM: *y = s_flo(mpz_get_d(c_big(*y)));   break;
                case SCM_TYPE_FIXNUM: *y = s_flo((flonum)c_fix(*y));      break;
            }
            break;
        case SCM_TYPE_BIGNUM:
            switch(ty) {
                case SCM_TYPE_FLONUM: *x = s_flo(mpz_get_d(c_big(*x)));   break;
                case SCM_TYPE_BIGNUM: /* no-op */                         break;
                case SCM_TYPE_FIXNUM: *y = s_bigi(c_fix(*y));             break;
            }
            break;
        case SCM_TYPE_FIXNUM:
            switch(ty) {
                case SCM_TYPE_FLONUM: *x = s_flo((flonum)c_fix(*x));      break;
                case SCM_TYPE_BIGNUM: *x = s_bigi(c_fix(*x));             break;
                case SCM_TYPE_FIXNUM: *x = s_bigi(c_fix(*x));
                                      *y = s_bigi(c_fix(*y));             break;
            }
            break;
    }
    return type_of(*x);
